...
class LinkedList{
	...
	public:
		...
		void Insert(N element);
		
		//맨 뒤의 원소를 삭제, 제일 나중에 들어온 원소 삭제  - LinkedList
		virtual bool Delete(N &element);
...
template <class N>
void LinkedList<N>::Insert(N element){
	//노드 생성
...
template <class N>
bool LinkedList<N>::Delete(N &element){

	if (first == 0)
		return false;
	
	Node<N> *current = first;
	Node<N> *previous = 0;
